...
<div>
    <main id="main">
        <section class="breadcrumbs" style="margin-top:100px!important">
            <div class="container">
                <div class="d-flex justify-content-between align-items-center">
                    <h2>Berita dari Area {{ ucwords(str_replace('-', ' ', request()->segment(2))) }}</h2>
                    <ol>
                        <li>
        ...
        <section class="inner-page">
            <div class="container">
                @if ($daftarBerita->count() > 0)
                    @foreach ($daftarBerita as $item)
                        <a
                            href="{{ url(env('APP_URL') . 'berita/' . $item->id) }}" class="text-dark"
                            ...
                            <div class="d-flex mb-4">
                                ...
                                <div class="ml-3">
                                    <p class="h5">{{ $item->judul_berita }}</p>
                    ...
                    @endforeach
                    {{ $daftarBerita->links('vendor.pagination.bootstrap-4') }}
                @else
                    <div class="text-center my-4 py-4">
                        <p class="h3 font-weight-bold mb-3">Maaf, belum ada berita untuk area ini</p>
                        <a class="lead" href="{{ url(env('APP_URL')) }}">Kembali ke beranda</a>
                    </div>
                @endif
            </div>
        </section>
